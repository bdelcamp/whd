...
MAINTAINER Brandon Delcamp "delcamp.brandon@gmail.com"
...
ADD webhelpdesk-12.3.0.471-1.x86_64.rpm.gz /webhelpdesk.rpm.gz
...
RUN sed -i 's/^PRIVILEGED_NETWORKS=[[:space:]]*$/PRIVILEGED_NETWORKS=172.17.42.1,200.50.1.*/g' /usr/local/webhelpdesk/conf/whd.conf
ADD run.sh /run.sh
